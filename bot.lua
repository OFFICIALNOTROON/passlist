-- #opened by @source_home
--- حمایت کنید :/
---- اوپن شده بود باز گذاشتمش بخاطر دوستان درخواسته درخواست کننده :/
...
http.TIMEOUT = 10
local bot_id = 916191111 -- ایدی سودو
sudo_users = {916191111} --- ایدی سودو
...
if redis:get("bot_on") then
  tdcli.editMessageText(chat_id, msg.id_, nil, 'ربات سلف شما هم اکنون فعال میباشد', 1, 'md')
...
if input:match("ربات خاموش") and is_sudo(msg) then
if not redis:get("bot_on") then
  tdcli.editMessageText(chat_id, msg.id_, nil, '**ربات سلف هم اکنون غیر فعال میباشد**', 1, 'md')
...
if input:match("دشمن") and reply_id and is_sudo(msg) then
  ...
  function setenemy_reply(extra, result, success)
    if redis:sismember("enemy:", result.sender_user_id_) then
      tdcli.editMessageText(chat_id, msg.id_, nil, 'یوزر هم اکنون دشمن است', 1, 'md')
    else
...
--[[if input:match("^inv$") and reply_id and is_sudo(msg) then
  ...
  elseif input:match("^inv (%d+)$") and is_sudo(msg) then
    ...
    if input:match("^kick$") and reply_id and is_sudo(msg) then
      ...
      if input:match("پوکر خاموش") and is_sudo(msg) then
